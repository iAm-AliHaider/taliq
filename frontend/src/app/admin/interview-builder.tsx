              ...
              <tbody className="divide-y divide-gray-100">
                {interviews.map(i => (
                  <tr key={i.id} className="hover:bg-gray-50">
                    <td className="px-4 py-2 font-mono text-gray-400">{i.ref}</td>
                    <td className="px-4 py-2 font-semibold text-gray-900">{i.candidate_name}</td>
                    <td className="px-4 py-2 text-gray-600">{i.position}</td>
                    <td className="px-4 py-2"><Badge text={i.stage?.replace("_", " ")} color="indigo" /></td>
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-1">
                        <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                          <div className="h-full bg-indigo-400 rounded-full" style={{ width: `${(i.current_question / i.total_questions) * 100}%` }} />
                        </div>
                        <span className="text-[9px] text-gray-400">{i.current_question}/{i.total_questions}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2">
                      {i.average_score ? <ScoreBar score={i.average_score} /> : <span className="text-gray-300">--</span>}
                    </td>
                    <td className="px-4 py-2">
                ...
                {interviews.length === 0 && (
                  <tr><td colSpan={8} className="px-5 py-8 text-center text-gray-400">No interviews yet. Start one from the "Start Interview" tab or use voice: "Start an interview with Ahmed for Software Engineer"</td></tr>
                )}
              </tbody>
            </table>
          </div>
