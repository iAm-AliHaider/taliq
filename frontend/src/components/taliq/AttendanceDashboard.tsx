...
export function AttendanceDashboard({ managerName, date, team }: Props) {
  ...
  const statusConfig: Record<string, { color: string; label: string; icon: string }> = {
    present: { color: "badge-emerald", label: "Present", icon: "🟢" },
    remote: { color: "badge-blue", label: "Remote", icon: "🏠" },
    late: { color: "badge-gold", label: "Late", icon: "⚠️" },
    absent: { color: "badge-red", label: "Absent", icon: "🔴" },
    on_leave: { color: "badge-gray", label: "On Leave", icon: "🏖️" },
  };
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="grid grid-cols-4 gap-px bg-gray-100 border-b border-gray-100">
        {[
          { label: "Present", value: present, color: "text-emerald-600", bg: "bg-white" },
          { label: "Late", value: late, color: "text-amber-600", bg: "bg-white" },
          { label: "Absent", value: absent, color: "text-red-600", bg: "bg-white" },
          { label: "Leave", value: onLeave, color: "text-gray-500", bg: "bg-white" },
        ].map(s => (
